...
//使用framelayout可以使最后一个child覆盖之前添加的children，也就是surface是最后一个
public class SwipeLayout extends FrameLayout {
    ...
    /**
     * 默认设置swiplayout的位置，如果放在右边，就需要向左边滑动
     */
    ...
    /**
     * 默认swiplayout的位置在右边 《默认触及到为右边》
     */
    private DragEdge mCurrentDragEdge = DefaultDragEdge;

    /**
     * Android提供的方法，解决了android中手势处理过于复杂的问题
     */
    ...
    /**
     *《将滑动的触及到的边缘和需要显示的View相结合，这样就可以直接找到需要显示的view》
     * 滑动到不同的方向对应到的不同view，这样就可以直接映射到相应的view
     */
    ...
    public enum ShowMode {
        LayDown,        //意思bottom view在surface view的下方，在显示的时候布局位置改变的是surface
        PullOut         //意思tottom view在surface view对应的方向布局方向上，在显示的时候布局位置改变的不仅仅是surface view还有bottom view
    }

    private float[] mEdgeSwipesOffset = new float[4];

    /**
     * Swip过程几种状态的监听事件（这个list列表应该和DragEdge.LEFT ... DragEdge.BOTTOM对应的）
     */
    private List<SwipeListener> mSwipeListeners = new ArrayList<>();
    /**
     * Deny监听事件（这个list列表应该和DragEdge.LEFT ... DragEdge.BOTTOM对应的）
     */
    ...
    /**
     * 当view打开了多少比例即认为需要打开这个view
     */
    private float mWillOpenPercentAfterOpen=0.75f;
    /**
     * 《当view关闭了多少比例即认为需要关闭这个view》
     */
    ...
    public SwipeLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        //设置SwapLayout所在的方向，初始化为空
        if ((dragEdgeChoices & DRAG_LEFT) == DRAG_LEFT) {       //设置SwapLayout在左边，为空
    ...
    /**
     * 添加swip过程状态监听事件（应该是一个dragview对应一个SwipListener）
     * @param l
     */
    ...
    /**
     * 删除相应的监听事件（应该是一个dragview对应一个SwipListener）
     * @param l
     */
    ...
    /**
     * 清空所有的监听事件（应该是一个dragview对应一个SwipListener）
     */
    ...
    /**
     * 添加deny监听事件（应该是一个dragview对应一个SwipListener）
     * @param denier
     */
    ...
    /**
     * 删除deny监听事件（应该是一个dragview对应一个SwipListener）
     * @param denier
     */
    ...
    /**
     * 清空deny监听事件（应该是一个dragview对应一个SwipListener）
     * @param denier
     */
    ...
    public void addRevealListener(int childId, OnRevealListener l) {
        ...
        if (!mShowEntirely.containsKey(child)) {
            mShowEntirely.put(child, false);      //默认不会全部显示
        }

        if (mRevealListeners.get(child) == null)  //添加显示监听函数
    ...
    /**
     * 连接ViewDragHelper与view之间的桥梁（这个view一般是指拥子view的容器即parentView）
     */
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            ...
            //边界处理
            if (child == getSurfaceView()) {        //当前显示的是surface view
                switch (mCurrentDragEdge) {
                    case Top:
                    case Bottom:
                        return getPaddingLeft();
                    case Left: //bottom view 在surface view的左边
                        if (left < getPaddingLeft()) return getPaddingLeft();   //surface view左边已经移出了parent view
                        if (left > getPaddingLeft() + mDragDistance)  //
                            return getPaddingLeft() + mDragDistance;
                        break;
                    case Right: //bottom view 在surface view的右边
                        if (left > getPaddingLeft()) return getPaddingLeft();
                        if (left < getPaddingLeft() - mDragDistance)
                            return getPaddingLeft() - mDragDistance;
                        break;
                }
            } else if (getCurrentBottomView() == child) {    //获取当前显示的bottom view是否滑动view相同
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (!isSwipeEnabled()) {            //不可以滑动，直接传递事件
            return false;
        }
    ...
    /**
     * surface view在parent view中的rect 坐标
     */
    private Rect hitSurfaceRect;

    /**
     * 当前事件坐标是否在surface view中
     * @param ev
     * @return
     */
    ...
    /**
     * 需要展示的为surface view，而且规定放在前面的必须为surface
     * surface必须放在最后一个
    ...
    /**
     * 这个应该是用来获取所有切换的view，也就是在surface下面的view
    ...
    /**
     * 获取所有的layout的view，包括left，top，right，bottom
    ...
    public void open(boolean smooth, boolean notify) {
        ...
        int dx, dy;
        Rect rect = computeSurfaceLayoutArea(true);   //获取surfaceview的位置（打开bottom view状态下的位置）
        if (smooth) {
     ...
     * 用来计算surface view在不同状态下的位置（即将需要显示的位置）
     * true：需要打开bottom view，需要计算surface view移动的位置
     * false：需要打开surface view
    ...
    /**
     * 用来计算bottom view需要显示的位置
     * @param mode
     * @param surfaceArea surface view即将显示的位置
     * @return
     */
